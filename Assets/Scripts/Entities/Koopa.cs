...
public class Koopa : CharacterController
{
    ...
    {
        if (groundObject == null || !ValidateReferences(CharacterCollider)) return false; // Dont turn around if we're already falling or have no collider
        Rigidbody2D _ = new();
        return !CheckForObject(Vector2.down, GroundCheckDistance, direction * CharacterCollider.bounds.size.x, ref _);
    }
